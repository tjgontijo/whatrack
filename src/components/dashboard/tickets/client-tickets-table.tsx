...
import { TicketCard } from '@/components/data-table/cards/ticket-card'
...
export default function ClientTicketsTable() {
  ...
  const { data, isLoading, isError } = useQuery<TicketsListResponse>({
    ...
    queryFn: async () => {
      ...
      return ticketsListResponseSchema.parse(await response.json())
    },
  ...
  const columns = React.useMemo<ColumnDef<TicketListItem>[]>(
    () => [
      { header: 'Criado em', accessorKey: 'createdAt', cell: ({ getValue }) => formatDateTime(getValue() as string) },
      { header: 'Status', accessorKey: 'status', cell: ({ getValue }) => getValue() ?? '—' },
      { header: 'Lead', accessorKey: 'leadName', cell: ({ getValue }) => getValue() ?? '—' },
      { header: 'Telefone', accessorKey: 'leadPhone', cell: ({ getValue }) => getValue() ?? '—' },
    ],
    []
  )
      ...
      <div className="hidden md:block">
        ...
        >
          <FilterBarSection title="Busca">
            <FilterInput value={input} onChange={setInput} placeholder="Pesquisar tickets..." />
          </FilterBarSection>
      ...
      <ResponsiveDataTable
        ...
        mobileCard={(row) => (
          <TicketCard
            ...
            pipefyId={row.original.pipefyId}
            pipefyUrl={row.original.pipefyUrl}
            ...
            leadInstagram={row.original.leadInstagram}
            leadMail={row.original.leadMail}
            onViewLead={(leadId) => {
              const ticket = items.find((t) => t.leadId === leadId)
              if (ticket) {
                setDialog({
                  leadId,
                  leadName: ticket.leadName,
                  leadPhone: ticket.leadPhone,
                })
              }
            }}
          />
