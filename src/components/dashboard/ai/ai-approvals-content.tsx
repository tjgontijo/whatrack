...
export function AiApprovalsContent() {
  ...
  const approveMutation = useMutation({
    mutationFn: async (id: string) =>
      apiFetch(`/api/v1/ai-insights/${id}/approve`, {
        method: 'PATCH',
        orgId: orgId!,
      }),
    onSuccess: () => {
  ...
  const rejectMutation = useMutation({
    mutationFn: async (id: string) =>
      apiFetch(`/api/v1/ai-insights/${id}/reject`, {
        method: 'PATCH',
        orgId: orgId!,
      }),
    onSuccess: () => {
  ...
  const formatDealValue = (value: unknown): string => {
  ...
  if (isLoading) {
    return <LoadingPage message="Analisando sugestões da IA..." />
  }
              ...
              <div className="mb-5 space-y-3">
                <div className="bg-background/80 flex items-center justify-between rounded-lg border p-3 shadow-sm">
                  <div>
                    ...
                    <p className="text-sm font-medium">{payload.itemName || 'Não especificado'}</p>
                  </div>
