...
function SortableCard({
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className="bg-card border-border group cursor-grab rounded-lg border p-3 shadow-sm transition-all hover:shadow-md active:cursor-grabbing"
    >
      {/* Card Header */}
      ...
      <div className="border-border flex items-center justify-between border-t pt-3">
        <div className="text-left">
          {lead.mail && (
            <p className="text-muted-foreground max-w-[120px] truncate text-xs" title={lead.mail}>
              {lead.mail}
            </p>
          )}
        </div>
...
export function LeadsKanbanBoard({
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    if (typeof overId === 'string' && COLUMNS.some((col) => col.id === overId)) {
      setLeadsWithStatus((prevLeads) =>
        prevLeads.map((lead) => (lead.id === activeId ? { ...lead, status: overId } : lead))
      )
  ...
  return (
    ...
    >
      <div className="flex h-full gap-4 overflow-x-auto pb-4">
        {COLUMNS.map((column) => {
          ...
          return (
            <div
              key={column.id}
              className="bg-muted/30 flex w-[320px] flex-shrink-0 flex-col rounded-xl"
            >
              {/* Column Header */}
              ...
              >
                <div
                  id={column.id}
                  className="scrollbar-thin min-h-[200px] flex-1 space-y-3 overflow-y-auto p-3"
                >
                  ...
                  {columnLeads.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-8 text-center">
                      <p className="text-muted-foreground text-sm">Nenhum lead</p>
                    </div>
                  )}
                </div>
              </SortableContext>
      ...
      <DragOverlay>
        {activeLead ? (
          <div className="bg-card border-border w-[320px] rounded-lg border p-3 opacity-90 shadow-lg">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <h4 className="truncate text-sm font-semibold">{activeLead.name || 'Sem nome'}</h4>
                <p className="text-muted-foreground font-mono text-xs">
