  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      {leads.map((lead) => {
        ...
        const firstOriginParts = [origin, getSourceLabel(source)].filter(Boolean)
        const firstOriginText = firstOriginParts.length > 0 ? firstOriginParts.join(' • ') : null

        return (
          <div
            key={lead.id}
            className="border-border bg-card hover:border-primary/40 group relative flex cursor-pointer flex-col rounded-xl border p-4 transition-all hover:shadow-md"
          ...
          >
            ...
            >
              <Button
                variant="ghost"
                size="icon"
                className="text-muted-foreground hover:bg-muted hover:text-primary h-6 w-6"
              ...
              <Button
                variant="ghost"
                size="icon"
                className="text-muted-foreground hover:bg-muted hover:text-destructive h-6 w-6"
              ...
              <div className="min-w-0 flex-1">
                ...
                {firstOriginText && (
                  <p
                    ...
                    title="Primeira origem do lead"
                  >
                    <MapPin className="mr-0.5 inline h-2.5 w-2.5 opacity-60" />
                    {firstOriginText}
                  </p>
                )}
              </div>
            ...
            {/* Contact & Meta Info */}
            <div className="text-muted-foreground mb-4 flex flex-col gap-1.5 text-[11px]">
            ...
            <div className="border-border/50 mt-auto border-t pt-3">
              <div className="flex items-center justify-between">
                <div className="text-muted-foreground flex items-center gap-1.5" title="Tickets">
