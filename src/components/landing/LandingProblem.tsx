  ...
  return (
    <section ref={ref} className='relative overflow-hidden bg-white py-32 dark:bg-zinc-950'>
      ...
      <div className='pointer-events-none absolute inset-0'>
        <div className='absolute top-0 right-0 h-full w-1/2 bg-gradient-to-br from-red-50 to-orange-50 dark:from-red-950/10 dark:to-orange-950/10' />
        <svg
      ...
      <div className='relative mx-auto max-w-[1400px] px-6 sm:px-8 lg:px-12'>
        ...
        >
          <div className='mb-6 inline-flex items-center gap-2 rounded-full border border-red-200 bg-red-50 px-4 py-2 dark:border-red-900/50 dark:bg-red-950/30'>
            ...
            <span className='font-semibold text-red-700 text-sm uppercase tracking-wider dark:text-red-400'>
              {content.badge}
            </span>
          </div>

          <h2 className='mb-6 font-bold font-geist text-3xl text-zinc-900 leading-tight tracking-tight sm:text-5xl dark:text-white'>
            {content.headline}{' '}
          ...
          {content.problems.map((problem, i) => {
            ...
            return (
              ...
              >
                ...
                {/* Content */}
                <h3
                  className={`relative mb-4 font-geist ${isLarge ? 'text-3xl' : 'text-2xl'} font-bold text-zinc-900 leading-tight dark:text-white`}
                >
                  {problem.title}
                </h3>

                <p
                  className={`relative ${isLarge ? 'text-lg' : 'text-base'} text-zinc-600 leading-relaxed dark:text-zinc-400`}
                >
                  {problem.description}
                </p>

                {/* Decorative corner */}
                <div className='pointer-events-none absolute -right-8 -bottom-8 h-24 w-24 rounded-full border-4 border-red-500/10 dark:border-red-500/20' />
              </motion.div>
            )
          })}
        </div>
        ...
        {content.closingText && (
          <motion.div
            ...
            className='mt-16 rounded-2xl border-red-500 border-l-4 bg-red-50 p-8 dark:bg-red-950/20'
          >
            <p className='text-lg text-zinc-700 italic leading-relaxed dark:text-zinc-300'>
              "{content.closingText}"
            </p>
          </motion.div>
        )}
