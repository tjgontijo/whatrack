...
export function LandingSolution({ content }: LandingSolutionProps) {
  ...
  return (
    ...
    >
      ...
      <div className='pointer-events-none absolute top-1/2 left-1/2 h-[800px] w-[800px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-gradient-to-r from-emerald-500/10 via-amber-500/10 to-emerald-500/10 blur-3xl' />
          ...
          {content.cards.map((card, i) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`pointer-events-none absolute top-0 right-0 h-64 w-64 bg-gradient-to-bl opacity-0 blur-3xl transition-opacity group-hover:opacity-100 ${colors.glow}`}
                />
                ...
                <div className='relative mb-6'>
                  ...
                  <div className='absolute -top-2 -right-2 grid grid-cols-2 gap-1 opacity-20'>
                    {[...Array(4)].map((_, idx) => (
                      <div key={idx} className={`h-1 w-1 rounded-full ${colors.icon}`} />
                    ))}
                  </div>
                </div>
                ...
                {/* Content */}
                <h3
                  className={`relative mb-4 font-bold font-geist text-zinc-900 leading-tight dark:text-white ${isLarge ? 'text-3xl lg:text-4xl' : 'text-2xl'}`}
                >
                  {card.title}
                </h3>

                <p
                  className={`relative text-zinc-700 leading-relaxed dark:text-zinc-300 ${isLarge ? 'text-lg' : 'text-base'}`}
                >
                  {card.description}
                </p>
                ...
                {isLarge && (
                  <div
                    className={`absolute -right-8 -bottom-8 h-32 w-32 rounded-full border-8 opacity-10 ${colors.border}`}
                  />
                )}
              </motion.div>
            )
