  ...
  return (
    <section ref={ref} className='relative overflow-hidden bg-zinc-950 py-32'>
      ...
      <div className='pointer-events-none absolute inset-0'>
        ...
        <div className='absolute top-1/2 -left-40 h-80 w-80 -translate-y-1/2 rounded-full bg-emerald-500/5 blur-3xl' />
        <div className='absolute top-1/2 -right-40 h-80 w-80 -translate-y-1/2 rounded-full bg-red-500/5 blur-3xl' />
      </div>
      ...
      <div className='relative mx-auto max-w-[1400px] px-6 sm:px-8 lg:px-12'>
        ...
        >
          <div className='mb-6 inline-flex items-center gap-2 rounded-full border border-zinc-700 bg-zinc-900 px-4 py-2'>
            ...
            <span className='font-semibold text-sm text-zinc-400 uppercase tracking-wider'>
              Comparativo
            </span>
          </div>

          <h2 className='font-bold font-geist text-3xl text-white leading-tight tracking-tight sm:text-5xl'>
            A diferença entre{' '}
        ...
        <div className='overflow-hidden rounded-3xl border border-zinc-800'>
          ...
          <div className='grid grid-cols-2'>
            <div className='flex items-center gap-3 border-zinc-800 border-r border-b bg-red-950/30 px-6 py-4 sm:px-8 sm:py-5'>
              <div className='flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-red-500/20'>
                <X className='h-4 w-4 text-red-400' />
              </div>
              <span className='font-bold font-geist text-red-400 text-sm uppercase tracking-wider sm:text-base'>
                Sem WhaTrack
              </span>
            </div>
            <div className='flex items-center gap-3 border-zinc-800 border-b bg-emerald-950/30 px-6 py-4 sm:px-8 sm:py-5'>
              <div className='flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-emerald-500/20'>
                <Check className='h-4 w-4 text-emerald-400' />
              </div>
              <span className='font-bold font-geist text-emerald-400 text-sm uppercase tracking-wider sm:text-base'>
                Com WhaTrack
              </span>
            </div>
          </div>
          ...
          {rows.map((row, i) => (
            ...
            >
              {/* Without */}
              <div
                className={`flex items-start gap-3 border-zinc-800 border-r px-6 py-5 sm:px-8 ${
              ...
              >
                ...
                <p className='text-sm text-zinc-500 leading-relaxed sm:text-base'>{row.without}</p>
              </div>

              {/* With */}
              <div
                className={`flex items-start gap-3 px-6 py-5 sm:px-8 ${
                  i < rows.length - 1 ? 'border-zinc-800 border-b' : ''
                } ${i % 2 === 0 ? 'bg-zinc-900/30' : 'bg-zinc-900/10'}`}
              >
                <div className='mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-500/10'>
                  <Check className='h-3.5 w-3.5 text-emerald-500' />
                </div>
                <p className='font-medium text-sm text-zinc-200 leading-relaxed sm:text-base'>
                  {row.with}
                </p>
              </div>
            </motion.div>
