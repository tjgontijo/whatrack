...
function StatCard({
  ...
  return (
    <div className='rounded-xl border p-4'>
      ...
      <p className={`mt-2 font-bold text-2xl ${muted ? 'text-muted-foreground' : ''}`}>{value}</p>
    </div>
  )
}
      ...
      {counters.cancelled > 0 && (
        <div className='flex items-center gap-2'>
          ...
          <p className='text-muted-foreground text-xs'>
            {counters.cancelled} campanha{counters.cancelled !== 1 ? 's' : ''} cancelada
            {counters.cancelled !== 1 ? 's' : ''}
          </p>
        </div>
      )}
    </div>
