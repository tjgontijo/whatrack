...
export async function dispatchAiEvent(
  ...
  try {
    ...
    const history = ticket.conversation.messages
      .reverse()
      .map(
        (m) => `[${m.direction === 'INBOUND' ? 'Cliente' : 'Atendente'}]: ${m.body || '[Mídia]'}`
      )
      .join('\n')
    ...
    for (const agentDef of agents) {
      ...
      if (recentInsight) {
        console.log(
          `[AI Copilot] Agent ${agentDef.name} already executed recently on Ticket ${ticketId}. Skipping.`
        )
        continue
      }
...
${history}`
    ...
    console.error(
      `[AI Copilot] Error dispatching event ${eventType} for ticket ${ticketId}:`,
      error
    )
    return 0
  }
