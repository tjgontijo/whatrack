...
export const campaignsColumns: ColumnDef<MetaCampaign>[] = [
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center gap-2">
          <div
            className={`h-2.5 w-2.5 rounded-full ${status.variant === 'success' ? 'bg-green-500' : 'bg-muted-foreground'}`}
          />
    ...
    cell: ({ row }) => {
      return (
        <div className="flex min-w-[200px] flex-col">
          <span
            className="text-foreground max-w-[280px] truncate text-sm font-medium"
            title={row.original.name}
          >
            {row.original.name}
          </span>
          <span className="text-muted-foreground text-xs">{row.original.accountName}</span>
        </div>
      )
    },
  },
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="text-primary text-center font-medium">{val ? val.toFixed(2) : '—'}</div>
      )
    },
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="text-center font-semibold">{row.original.metaPurchases}</div>
    ),
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div
          className={`text-center font-medium ${isNegative ? 'text-red-500' : 'text-green-600 dark:text-green-500'}`}
        >
          {formatCurrency(val)}
        </div>
      )
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="text-center">{formatCurrency(row.original.metaRevenue)}</div>
    ),
  },
  {
    accessorKey: 'ctr',
...

  {
    ...
    cell: ({ row }) => (
      <div className="text-center">{row.original.impressions.toLocaleString()}</div>
    ),
  },
  {
    accessorKey: 'cpm',
