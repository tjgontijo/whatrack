  ...
  try {
    ...
    if (!token) {
      ...
      return NextResponse.json(
        {
          error:
            'No access token available. Please ensure META_ACCESS_TOKEN is configured or provide an authorization code.',
        },
        { status: 500 }
      )
    }
    ...
    if (phoneNumberId) {
      ...
      try {
          ...
          return NextResponse.json(
            {
              ...
              availablePhones: phones.map((p: any) => ({
                id: p.id,
        ...
        return NextResponse.json(
          { error: `Erro ao buscar detalhes do número: ${err.message}` },
          { status: 502 }
        )
      }
    } else {
      ...
      try {
        const phones = await MetaCloudService.listPhoneNumbers({ wabaId, accessToken: token })
        console.log(
          `[ClaimWaba] Números encontrados (${phones.length}):`,
          phones.map((p: any) => p.display_phone_number)
        )
        ...
        if (phones.length === 0) {
          ...
          return NextResponse.json(
            {
              error:
                'Nenhum número de telefone encontrado na WABA. Configure um número no WhatsApp Business Manager.',
            },
            { status: 400 }
          )
        }
        ...
        return NextResponse.json(
          { error: `Erro ao buscar números de telefone da WABA: ${err.message}` },
          { status: 502 }
        )
      }
    }
    ...
    return NextResponse.json({ error: error.message || 'Failed to claim WABA' }, { status: 500 })
  }
}
