...
2. **Erro de Autenticação**: A Vercel envia automaticamente o Header `Authorization: Bearer <CRON_SECRET>`. Porém, as suas rotas aguardam o secret no *query parameter* (`?secret=`) ou num header customizado chamado `x-cron-secret`. Isso resultaria invariavelmente em Erro 401 (Unauthorized).
