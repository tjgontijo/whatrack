...
|------|---------|---------|
| CAPI service | `src/features/meta-ads/services/capi.service.ts` | Busca ticket, hash phone, envia, loga em MetaConversionEvent |
| Hook de mudanca de fase | `ticket.service.ts:592` | `updateTicketAndTrackCapi` chama `triggerStageCapiEvent` apos update |
| Dedup no banco | `MetaConversionEvent @@unique([ticketId, eventName])` | Existe, precisa adicionar `pixelId` |
...
**Problema:** Nao existe vinculo configuravel entre `TicketStage` e pixels/eventos. Nao existe tabela de tipos de eventos padrao da Meta.

**Solucao:**
1. Criar `MetaEventType` — lookup de eventos padrao com seed.
2. Criar `TicketStageMetaRule` com `stageId`, `pixelId`, `eventName`, `fireOnce`.
3. Adicionar `pixelId` ao `MetaConversionEvent` e atualizar unique constraint para `[ticketId, pixelId, eventName]`.
4. Adicionar relacoes em `TicketStage` e `MetaPixel`.

---

### 🔴 P2: `updateTicketStageAction` bypassa o hook CAPI

**Problema:** Server Action do drag-and-drop do kanban (`tickets/actions/update-ticket-stage-action.ts:30`) faz `prisma.ticket.update` diretamente — nenhum evento CAPI dispara hoje ao mover cards.

**Solucao:** Substituir `prisma.ticket.update` por `updateTicketAndTrackCapi`.

---

### 🟡 P3: Logica de disparo usa heuristica de nome

**Problema:** `getCapiEventForStage` em `ticket.service.ts:315` mapeia nome da fase para evento por substring match. Nao configuravel, nao suporta multiplos pixels.
...
**Problema:** Sem interface para criar/editar `TicketStageMetaRule`.
...
|---|---|
| Hook de fase nao existe (T2) | Existe — `updateTicketAndTrackCapi` em `ticket.service.ts:592` |
...
|----------|------------|---------|
| P1: Schema (MetaEventType + TicketStageMetaRule + MetaConversionEvent) | Alto | 1.5h |
